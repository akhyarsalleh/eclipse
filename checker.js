window.runLicenseChecker = function(callback) {
    ...
    // Field-level keywords that indicate non-expiring or static information
    ...
    // Specific issue-date terms (ignored ONLY for issue date fields/columns)
    ...
    // Returns the table column header for a given element if inside a table
    ...
    // Retrieves primary descriptor from the first column of the same row (e.g. "CPL(A)")
    ...
    // Gathers surrounding DOM text context
    ...
    function findLabelText(el) {
        // Try getting row lead label (e.g., CPL(A))
    ...
    for (var i = 0; i < elements.length; i++) {
        ...
        if (el.children.length === 0 && el.textContent.trim().length > 0) {
            ...
            if (isRed || isDate || text.toUpperCase() === 'EXPIRED') {
                ...
                // 2. Field keyword check: Ignore static field types (e.g. Date of Birth)
                if (!isException) {
                    for (var k = 0; k < IGNORED_FIELD_KEYWORDS.length; k++) {
                        if (surroundingContext.includes(IGNORED_FIELD_KEYWORDS[k])) {
                            isException = true;
                            break;
                        }
                    }
