...
function parseAndBuildDashboard(html, THRESHOLD) {
    ...
    // Helper: Fetch direct children of row
    ...
    function parseLicenseDate(dateStr) {
        ...
        var day = parseInt(match[10], 10);
        var monthStr = match[11].toUpperCase(); 
        var year = parseInt(match[12], 10);     
    ...
    var nameMatch = bodyText.match(/Full Name of Holder\s*(?:\([^)]*\)\s*)*\s*\|*\s*\|*\s*([A-Za-z\s\.\'\-]+?)(?:\s+IVc|\s+Date of|\s*\||$)/i);
    if (nameMatch && nameMatch[10]) {
        pilotName = nameMatch[10].trim();
    }

    var licMatch = bodyText.match(/LICENCE NO\s*(?:Nombor Lesen Baru\s*)?(\w+)/i);
    if (licMatch && licMatch[10]) {
        pilotLicense = "Licence No: " + licMatch[10].trim();
    }
